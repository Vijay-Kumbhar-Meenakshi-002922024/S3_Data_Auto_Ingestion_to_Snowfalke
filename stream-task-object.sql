...
-- create 3 streams for orders, customer, item, in append only mode
...
create or replace task order_curated_tsk
    ...
    schedule = '1 minute'
    when
    system$stream_has_data('landing_zone.landing_order_stm')
    as
    merge into curated_zone.curated_order curated_order
    using landing_zone.landing_order_stm landing_order_stm on
...
create or replace task customer_curated_tsk
          ...
          schedule  = '2 minute'
      when
          system$stream_has_data('landing_zone.landing_customer_stm') AND system$stream_has_data('landing_zone.landing_order_stm')
      as
      merge into curated_zone.curated_customer curated_customer 
      using landing_zone.landing_customer_stm landing_customer_stm on
...
create or replace task item_curated_tsk
          ...
          schedule  = '3 minute'
      when
          system$stream_has_data('landing_zone.landing_item_stm')
      as
      merge into curated_zone.curated_item item using landing_zone.landing_item_stm landing_item_stm on
